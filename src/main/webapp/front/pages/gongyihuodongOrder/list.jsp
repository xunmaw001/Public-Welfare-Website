...
<div id="app">
    ...
    <div class="line-container">
        <p class="line" style="background: #EEEEEE;"> 我的公益活动预约 </p>
    </div>
    <!-- 标题 -->
        ...
        <div class="right-container" style="padding-top: 0;">
            <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
                <%--<ul class="layui-tab-title">--%>
                    <%--<li v-for="item in gongyihuodongOrderTypesList" :value="item.codeIndex"  @click="pageList(item.codeIndex)">{{item.indexName}}</li>--%>
                ...
                <div class="layui-tab-content">
                    <table class="layui-table" lay-skin="nob">
                        ...
                        <tr v-for="(item,index) in dataList" v-bind:key="index">
                            <td style="width: 60px;">{{item.gongyihuodongName}}</td>
                            <td style="width: 80px;">{{item.gongyihuodongValue}}</td>
                            <td style="width: 250px;">
                                <img :src="item.gongyihuodongPhoto" style="width: 100px;height: 100px;object-fit: cover;">
                            </td>
                            <td style="width: 60px;">{{item.canyuTime}}</td>
                            <td style="width: 80px;">{{item.gongyihuodongOrderValue}}</td>
...
<script>
    var vue = new Vue({
        ...
        filters: {
            //把 gongyihuodongOrderContent 字段去除无用字符 如果字段长度大于60  就截取到60
            gongyihuodongOrderContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            //列表查询
            ,pageList(gongyihuodongOrderTypes) {
                // this.gongyihuodongOrderTypes = gongyihuodongOrderTypes;
                // 获取列表数据
                layui.http.request('gongyihuodongOrder/list', 'get', {
                    page: 1,
                    limit: this.limit,
                    // gongyihuodongOrderTypes: vue.gongyihuodongOrderTypes
                }, function(res) {
                        ...
                        jump: function(obj, first) {
                            ...
                            if (!first) {
                                layui.http.request('gongyihuodongOrder/list', 'get', {
                                    page: obj.curr,
                                    limit: obj.limit,
                                    // gongyihuodongOrderTypes: this.gongyihuodongOrderTypes
                                }, function(res) {
            ...
            ,pay(id) {
                // 获取商品详情信息
                layui.http.requestJson(`gongyihuodongOrder/pay`, 'PPST',{
                    id:id
                }, (res) => {
                    if(res.code==0){
                        layer.msg('支付成功', {
            ...
            ,refund(id) {
                // 获取商品详情信息
                layui.http.request(`gongyihuodongOrder/refund?id=`+id, 'get', {}, (res) => {
                    if(res.code==0){
                        layer.msg('退款成功', {
                            time: 2000,
                            icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        // 获取列表数据
        http.request('gongyihuodongOrder/list', 'get',{
            page: 1,
            limit: limit,
            // gongyihuodongOrderTypes: vue.gongyihuodongOrderTypes
        }, function(res) {
                ...
                jump: function(obj, first) {
                    ...
                    if (!first) {
                        http.request('gongyihuodongOrder/list', 'get', {
                            page: obj.curr,
                            limit: obj.limit,
                            gongyihuodongOrderType: vue.gongyihuodongOrderType
                        }, function(res) {
                            vue.dataList = res.data.list
                        })
                    }
       ...
       function gongyihuodongOrderTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=gongyihuodong_order_types", 'get', {}, function (res) {
               if(res.code == 0){
