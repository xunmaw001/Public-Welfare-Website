            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">公益活动收藏展示</span>
                    ...
                    <form class="layui-form" lay-filter="myForm">
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="gongyihuodongName" id="gongyihuodongName" placeholder="活动名称" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <select name="gongyihuodongTypes" id="gongyihuodongTypes" lay-filter="gongyihuodongTypes">
                                      <option value="">--请选择--</option>
                                      <option v-for="item in gongyihuodongTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                                  </select>
                              </div>
                          </div>
                          ...
                          <div class="layui-inline" style="margin-bottom: 10px;">
                              ...
                              <div class="layui-input-inline">
                                  <input type="text" name="canyuTime" id="canyuTime" placeholder="参与时间" autocomplete="off" class="layui-input">
                              </div>
                          </div>
                        ...
                        <ul>
                            <li v-for="(item,index) in dataList" v-bind:key="index">
                                <div class="ng-prslist-doc">
                                    <div class="ng-prslist-img" @click="jump('../gongyihuodong/detail.jsp?id='+item.gongyihuodongId)">
                                        <a href="#" class="ng-prsliik">
                                          <span class="ng-prsliik-ki ng-prsliik-ki_1">
                                            <img class="swiper-item" :src="item.gongyihuodongPhoto?item.gongyihuodongPhoto.split(',')[0]:''" />
                                          </span>
                                    ...
                                    <div class="ng-prslist-tit" @click="jump('../gongyihuodong/detail.jsp?id='+item.gongyihuodongId)"><span>{{item.gongyihuodongName}}</span></div>
                                    <div class="ng-prslist-tit" v-if="item.gongyihuodongNewMoney"><span style="color: red;">{{item.gongyihuodongNewMoney}} RMB</span></div>
                                    <div class="ng-prslist-tit" ><span style="color: red;" @click="deleteGongyihuodongCollection(item.id)">删除 {{item.gongyihuodongName}} 收藏</span></div>
                                </div>
                            </li>
                        </ul>
                    </div>
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            gongyihuodongCollectionTypes: 1,//类型
            limit: 10,
        ...
        filters: {
            //把 gongyihuodongCollectionContent 字段去除无用字符 如果字段长度大于60  就截取到60
            gongyihuodongCollectionContent: function(val) {
                if (val) {
        ...
        methods: {
            ...
            ,deleteGongyihuodongCollection(id){
                layui.layer.confirm('确认要删除这个收藏吗？', {
                    btn : [ '确定', '取消' ]//按钮
                }, function(index) {
                    var paramArray = [];
                    paramArray.push(id);
                    layui.http.requestJson('gongyihuodongCollection/delete', 'post',paramArray, function (res) {
                        if (res.code == 0) {
                            layer.msg('删除成功', {
                                time: 2000,
                                icon: 6
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#gongyihuodongName').val() != null && jquery('#gongyihuodongName').val() != "" && jquery('#gongyihuodongName').val() != "null" ) {
                param['gongyihuodongName'] = jquery('#gongyihuodongName').val();
            }
                     
            var gongyihuodongTypes = $("#gongyihuodongTypes").find("option:selected").val();
            if (gongyihuodongTypes != null && gongyihuodongTypes != "" && gongyihuodongTypes != "null") {
                            param['gongyihuodongTypes'] = gongyihuodongTypes;
                        }
                                                             
            if (jquery('#canyuTime').val() != null && jquery('#canyuTime').val() != "" && jquery('#canyuTime').val() != "null" ) {
                param['canyuTime'] = jquery('#canyuTime').val();
            }
                                        
            // 当前表
                                                    // 获取列表数据
            http.request('gongyihuodongCollection/list', 'get', param, function(res) {
...
// 级联表的字典表
       ...
       function gongyihuodongTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=gongyihuodong_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.gongyihuodongTypesList = res.data.list;
               }
           });
       }
