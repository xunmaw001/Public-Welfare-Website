            ...
            <div class="ng-inmod-ini">
                <div class="ng-fault ng-container">
                    <div class="ng-inmod-the ng-container ng-bg-fff">
                        ...
                        <span class="ng-inmod-the-cn" aos="fade-down">民生新闻展示</span>
                ...
                <fieldset class="search-container" style="text-align: center;">
                    <form class="layui-form" lay-filter="myForm">
                      ...
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">民生新闻名称</label>
                          <div class="layui-input-inline">
                              <input type="text" name="minshengxinwenName" id="minshengxinwenName" placeholder="民生新闻名称" autocomplete="off" class="layui-input">
                          </div>
                      </div>
     
                      <div class="layui-inline" style="margin-bottom: 10px;">
                          <label class="layui-form-label">民生新闻类型</label>
                          <div class="layui-input-inline">
                              <select name="minshengxinwenTypes" id="minshengxinwenTypes" lay-filter="minshengxinwenTypes">
                                  <option value="">--请选择--</option>
                                  <option v-for="item in minshengxinwenTypesList" :value="item.codeIndex" :key="item.codeIndex">{{ item.indexName }}</option>
                              </select>
                          </div>
                      </div>
                        ...
                        <div class="article-list">
                            <ul class="after">
                                <li v-for="(item,index) in dataList" v-bind:key="index">
                                    <a target="_self" :href="'../minshengxinwen/detail.jsp?id='+item.id">
                                        <img width="180" class="fl" :src="item.minshengxinwenPhoto" />
                                        <h5>{{item.minshengxinwenName}}</h5>
                                        <p class="time-sour">
                                            <span>发布时间：{{item.insertTime}} </span>
                                        </p>
                                        <p class="reamk">{{item.minshengxinwenContent|minshengxinwenContent}}...</p>
                                    </a>
                                </li>
                            </ul>
                            <div class="pages">
...
<script>
    var vue = new Vue({
        ...
        data: {
            ...
            dataList: [],//数据
        // 当前表字典表数据
            minshengxinwenTypesList : [],
            minshengxinwenTypes: 1,//类型
            limit: 10,
        ...
        filters: {
            //把 minshengxinwenContent 字段去除无用字符 如果字段长度大于60  就截取到60
            minshengxinwenContent: function(val) {
                if (val) {
    ...
    layui.use(['layer', 'element', 'carousel', 'laypage','form', 'http', 'jquery', 'laydate'], function() {
        ...
        function pageList() {
            ...
            if (jquery('#minshengxinwenName').val() != null && jquery('#minshengxinwenName').val() != "" && jquery('#minshengxinwenName').val() != "null" ) {
                param['minshengxinwenName'] = jquery('#minshengxinwenName').val();
            }
         
            var minshengxinwenTypes = $("#minshengxinwenTypes").find("option:selected").val();
            if (minshengxinwenTypes != null && minshengxinwenTypes != "" && minshengxinwenTypes != "null") {
                param['minshengxinwenTypes'] = minshengxinwenTypes;
            }
                                            // 获取列表数据
            http.request('minshengxinwen/list', 'get', param, function(res) {
    ...
    // 当前表的字典表
       ...
       function minshengxinwenTypesSelect() {
           http.request("dictionary/page?page=1&limit=100&sort=&order=&dicCode=minshengxinwen_types", 'get', {}, function (res) {
               if(res.code == 0){
                   vue.minshengxinwenTypesList = res.data.list;
               }
           });
       }
